...
function disabledEndDate(current: Dayjs): boolean {
    ...
    return !!current && current < startTimePicker.value.startOf('day');
}
...
async function handleSubmit() {
    ...
    if (formState.value.originalPrice <= 0) return message.error('请输入正确的原价');
    ...
    if (!startTimePicker.value) return message.error('请选择活动开始时间');
    if (!endTimePicker.value) return message.error('请选择活动结束时间');
    if (endTimePicker.value.isBefore(startTimePicker.value)) return message.error('结束时间不能早于开始时间');

    if (formState.value.expireHours <= 0) return message.error('请输入正确的成团过期时间');

    formState.value.startTime = startTimePicker.value.format('YYYY-MM-DDTHH:mm:ss');
    formState.value.endTime = endTimePicker.value.format('YYYY-MM-DDTHH:mm:ss');
                    ...
                    <a-row :gutter="24">
                        <a-col :span="12">
                            <a-form-item label="活动名称" required>
                                <a-input v-model:value="formState.name" placeholder="请输入活动名称" :maxlength="100"
                                         show-count/>
                            </a-form-item>
                        </a-col>
                        <a-col :span="12">
                            <a-form-item label="成团过期时间" required>
                                <a-input-number v-model:value="formState.expireHours" :min="1" :max="168"
                                                style="width: 100%" addon-after="小时"/>
                            </a-form-item>
                        </a-col>
                    </a-row>
                        ...
                        <a-col :span="24">
                            <div class="selected-product-info">
                                <div class="sku-selection">
                                    ...
                                    <div v-else>
                                        <a-alert message="该商品暂无 SKU，可手动填写价格" type="info" :show-icon="true"/>
                                    </div>

                                    <div v-if="formState.skuId" class="sku-price-info">
                                        <a-alert message="已选择规格，价格已自动填充" type="success" :show-icon="true"/>
                                    </div>
                                </div>
                            </div>
                        </a-col>
                    ...
                    <a-row :gutter="24">
                        <a-col :span="12">
                            <a-form-item label="原价（分）" required>
                                <a-input-number v-model:value="formState.originalPrice" :min="1" style="width: 100%"/>
                            </a-form-item>
                        </a-col>
                        <a-col :span="12">
                            <a-form-item label="团购价（分）" required>
                                <a-input-number
                                    ...
                                    :min="1"
                                    :max="Math.max(1, formState.originalPrice - 1)"
                                    style="width: 100%"
                                />
                            </a-form-item>
                        </a-col>
                    ...
                    <a-row :gutter="24">
                        <a-col :span="8">
                            <a-form-item label="最小成团人数" required>
                                <a-input-number v-model:value="formState.minPeople" :min="2" :max="100"
                                                style="width: 100%"/>
                            </a-form-item>
                        </a-col>
                        <a-col :span="8">
                            <a-form-item label="最大成团人数" required>
                                <a-input-number v-model:value="formState.maxPeople" :min="formState.minPeople"
                                                :max="100" style="width: 100%"/>
                            </a-form-item>
                        </a-col>
                        <a-col :span="8">
                            <a-form-item label="自动成团">
                                <a-switch v-model:checked="formState.autoGroupEnabled" :checked-value="1"
                                          :unchecked-value="0"/>
                            </a-form-item>
                        </a-col>
                    </a-row>
